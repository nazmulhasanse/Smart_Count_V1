...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$data = json_decode($decodedData, true);
        ...
        try {
            ...
            // use query() method for MySQLi
            if ($conn->query($sql) === TRUE) {
                if($qty_type == 'fail' && isset($_POST['dataArray'])) {
                ...
                if ($data !== null) {
                    ...
                    foreach ($data as $item) {
                        ...
                        $date = date('Y-m-d H:i:s');
                        $sql_defect = "INSERT INTO `ework_defect` (`OperationID`, `StepNumber`, `Sewing_Process`, `PartName`, `DefectReason`, `No_of_Defect`, `ReMarks`, `Entry_dateime`, `Delete_datetime`, `Update_datetime`, `Status`) 
