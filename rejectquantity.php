...
<?php  exit;}
    ...
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('reject') AND ework_id < (SELECT MAX(ework_id) FROM eworker_operation)),
        0
    ) AS total_qty,
    COALESCE(
        (SELECT Qty FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('reject') ORDER BY ework_id DESC LIMIT 1),
        0
        ) AS last_row_qty,
    COALESCE(
        (SELECT sum(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('reject')),
        0
    ) AS total_sum_qty
    FROM eworker_operation
    WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('reject')
...
<html lang="en">
    ...
    <body>
        <div class="container">
            ...
            <form class="login-form" action="reject_data_store.php" id='formId' method="POST">
                <div class=" body">
                    ...
                    <ul>
                        ...
                        <?php else: ?>
                        <li>
                            <input type="hidden" id="qty_type" name="qty_type" class="number-input" value="reject">
            ...
            <div class="footer">
                ...
                <form method="post" action="reject_data_store.php">
