...
$sql_qty = "SELECT ework_id, color, size, end_datetime,
    ...
    if ($result_qty->num_rows > 0) {
        ...
        while($row_qty = $result_qty->fetch_assoc()) {
         $data_qty = $row_qty;
         $submitType =  empty($data_qty['end_datetime']) || is_null($data_qty['end_datetime']) ? 'end' : 'start';
...
<html lang="en">
    ...
    ?>
    <input type="hidden" id="ework_id" name="ework_id" class="number-input" value="<?php echo $data_qty['ework_id'] ?? ''; ?>">
