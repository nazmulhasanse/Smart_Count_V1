...
if ($result_defect->num_rows > 0) {
   $defet_data = [];
    ...
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type = 'fail' AND ework_id < (SELECT MAX(ework_id) FROM eworker_operation)),
        0
    ) AS total_qty,
    COALESCE(
        (SELECT Qty FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type = 'fail' ORDER BY ework_id DESC LIMIT 1),
        0
        ) AS last_row_qty,
    COALESCE(
        (SELECT sum(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type = 'fail'),
        0
    ) AS total_sum_qty
    FROM eworker_operation
    WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type = 'fail'
                    ...
                    <tr class="editable-row">
                        ...
                        <td>
                            <a href="#" class="edit-btn" onclick="toggleEdit(this)"><i
                                    style="color: blue; font-size: 17px;" class="fa fa-edit"></i></a>
                            <a href="#" class="save-btn" style="display: none;" onclick="saveEdit(this)"><i
                                    style="color: green; font-size: 17px;" class="fa fa-check"></i></a>
                            <a href="#" class="delete-btn" onclick="removeValue(this)"><i
                                    style="color: red; font-size: 17px;" class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
