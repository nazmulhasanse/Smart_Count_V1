...
<?php  exit;}
    ...
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('rework-reject') AND ework_id < (SELECT MAX(ework_id) FROM eworker_operation)),
        0
    ) AS total_qty,
    COALESCE(
        (SELECT Qty FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('rework-reject') ORDER BY ework_id DESC LIMIT 1),
        0
        ) AS last_row_qty,
    COALESCE(
        (SELECT sum(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('rework-reject')),
        0
    ) AS total_sum_qty
    FROM eworker_operation
    WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='R' and qty_type IN ('rework-reject')
    ...
    <body>
        <div class="container">
            ...
            <div class="header">
                ...
                <div></br></br></br>
                    <label style="font-size:25px"><strong><?php echo 'Re-Work Reject process is going on...'; ?></strong></label>
                </div>
            ...
            if ($data['Position'] == '2') {
                ...
                $sql_RejectReasonCodes = "SELECT CONCAT(Code, ' - ', Description) AS Code FROM `ework_mrd_library` WHERE LibraryName ='ReworkReasonCodes';"; // RejectReason
                $result_RejectReasonCodes = $conn->query($sql_RejectReasonCodes);

                if ($result_RejectReasonCodes->num_rows > 0) {
                    $data_RejectReasonCodes = [];
                    while ($row = $result_RejectReasonCodes->fetch_assoc()) {
                        $data_RejectReasonCodes[] = $row;
                    }
                } else {
                    echo "0 results for RejectReasonCodes";
                }
            ...
            var availableRejectReasonCodes =
                <?php echo !empty($data_RejectReasonCodes) ? json_encode(array_column($data_RejectReasonCodes, 'Code')) : '[]'; ?>;
            ...
            function addValue() {
                ...
                var selectedRejectReasonCode = document.getElementById("SelectRejectReasonCodes").value;
                ...
                // if (!validateInput(selectedRejectReasonCode, availableRejectReasonCodes)) {
                ...
                var value = {
                    ...
                    'RejectReason': selectedRejectReasonCode,
                    'Remarks': Remarks,
            ...
            function updateDisplay() {
                ...
                container.innerHTML =
                    "<tr><th>Reject Reason</th><th>Remarks</th><th>Action</th></tr>";
                dataArray.forEach(function(value) {
                    ...
                    row.innerHTML =
                        ...
                        "<td><a href='#' class='delete-btn' onclick='removeValue(this)'><i style='color: red;font-size: 20px;' class='fa fa-trash-o'></i></a></td>";
            ...
            function initAutocomplete() {
                ...
                //     minLength: 0,
                // })
                $("#SelectRejectReasonCodes").autocomplete({
                    source: availableRejectReasonCodes,
                    minLength: 0,
                }).focus(function() {
            ...
            <div class="select-container">
                ...
                <input type="text" name="SelectRejectReasonCodes" class="select-box" id="SelectRejectReasonCodes"
                    placeholder="Reject Reason" autocomplete="off" require>
                <input type="text" name="Remarks" class="select-box" id="Remarks" placeholder="Remarks"
                    autocomplete="off" require>
            ...
            <div class="select-container">
                <table id="addValueTable" class="table">
                    ...
                    <tr>
                        <td><?php echo $value['SelectRejectReasonCodes']; ?></td>
                        <td><?php echo $value['Remarks']; ?></td>
                        <td><a href="#" class="delete-btn" onclick="removeValue(this)"><i
                                    style="color: red;font-size: 20px;" class="fa fa-trash-o"></i></a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </table>
            ...
            <form class="login-form" action="rework_reject_data_store.php" id='formId' method="POST">
                <div class=" body">
                    ...
                    <ul>
                        ...
                        <?php else: ?>
                        <li>
                            <input type="hidden" id="qty_type" name="qty_type" class="number-input" value="rework-reject">
            ...
            <div class="footer">
                ...
                <form method="post" action="rework_reject_data_store.php">
            ...
            <div class="">
                <a class="button pass-button btn-home" href="reworkquantity.php"> <i class="fa fa-home"
        ...
        function confirmDelete() {
            if (confirm("Are you sure you want to delete this item?")) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'rework_reject_data_store.php',
        ...
        $(document).ready(function() {
            $("#formId").submit(function(ev) {
                ...
                // let SelectRejectReasonCodes = $("#SelectRejectReasonCodes").val();
                // let qty = $("#qty").val();

                // if (!SelectPartCode || !SelectRejectReasonCodes || !qty) {
                //     alert("Please select PartCode and RejectReason.");
                //     return;
                // }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var lastId = JSON.parse(response['ework_id']);
                        window.location.replace("rejectquantity.php?last_id=" + lastId);
                    },
