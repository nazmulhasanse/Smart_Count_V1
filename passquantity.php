...
$sql_qty = "SELECT ework_id, color, size, end_datetime,
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass') AND ework_id < (SELECT MAX(ework_id) FROM eworker_operation)),
        0
    ) AS total_qty,
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass') AND DateTime BETWEEN '$loginDatetime' AND '$endDatetime'),
        0
    ) AS total_hour_qty,
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass') AND DateTime BETWEEN '$startDatetime' AND '$loginDatetime'),
        0
    ) AS total_last_hour_qty,
    COALESCE(
        (SELECT Qty FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass') ORDER BY ework_id DESC LIMIT 1),
        0
    ) AS last_row_qty,
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass')),
        0
    ) AS total_sum_qty,
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass') AND DATE(DateTime) = '$currentDate'),
        0
    ) AS today_total_qty
FROM eworker_operation
WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' AND Status ='A' AND qty_type IN ('pass')
    ...
    <body>
        <div class="container">
            ...
            <form class="login-form" action="pass_data_store.php" id='formId' method="POST">
                ...
                <div class="footer">
                    <?php if (!isset($_SESSION['form_submitted'])) : ?>
                    <button class="button pass-button " id='submitButton1' type="submit" value="Submit">Submit</button>
                    <?php endif; ?>
                </div>
                ...
                <div class="footer">
                    ...
                    <button class="button pass-button " id='submitButton2' type="submit" value="Submit">Start</button>
                </div>
                <?php   }else{ ?>
                ...
                <div class="footer">
                    <input type="hidden" id="submit_type" name="submit_type" class="number-input" value="start">
                    <button class="button pass-button " id='submitButton3' type="submit" value="Submit">Start</button>
                </div>
                <?php } else {?>
                <div class="footer">
                    ...
                    <button class="button pass-button " id='submitButton4' type="submit" value="Submit"
        ...
        form.addEventListener("submit", function (event) {
           ...
           <?php if ($data['Position'] == '2') {?>
            <div class="footer">
                ...
                <form method="post" action="passquantity_edite.php">
