...
<?php  exit;}
    ...
    COALESCE(
        (SELECT SUM(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type IN ('rework-fail') AND ework_id < (SELECT MAX(ework_id) FROM eworker_operation)),
        0
    ) AS total_qty,
    COALESCE(
        (SELECT Qty FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type IN ('rework-fail') ORDER BY ework_id DESC LIMIT 1),
        0
        ) AS last_row_qty,
    COALESCE(
        (SELECT sum(Qty) FROM eworker_operation WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type IN ('rework-fail')),
        0
    ) AS total_sum_qty
    FROM eworker_operation
    WHERE cardnumber = '$cardnumber' AND WorkOrder = '$workorder' and Status ='A' and qty_type IN ('rework-fail')
    ...
    <body>
        <div class="container">
            ...
            <div class="header">
                ...
                <div></br></br></br>
                    <label style="font-size:25px"><strong><?php echo 'Re-Work Fail process is going on...'; ?></strong></label>
                </div>
                <div class="header-label">
            ...
            <form class="login-form" action="rework_fail_data_store.php" id='formId' method="POST">
                <div class=" body">
                    ...
                    <ul>
                        ...
                        <?php else: ?>
                        <li>
                            <input type="hidden" id="qty_type" name="qty_type" class="number-input" value="rework-fail">
            ...
            <div class="footer">
                ...
                <form method="post" action="reworkfailquantity_edite.php">
        ...
        function confirmDelete() {
            if (confirm("Are you sure you want to delete this item?")) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'rework_fail_data_store.php',
            ...
            $("#formId").submit(function(ev) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        window.location.replace("reworkfailquantity_edite.php?last_id=" + lastId);
                    },
